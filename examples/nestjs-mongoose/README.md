...
- [`test/app.e2e-spec.ts`](./test/app.e2e-spec.ts) — signs a user up over real HTTP, creates a post as that authenticated user, fetches it back, and asserts the populated `author.email` matches. This runs in CI on every push (see the root [`ci.yml`](../../.github/workflows/ci.yml)) — it's not just a README claim.
