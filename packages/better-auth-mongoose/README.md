...
[`examples/nestjs-mongoose`](../../examples/nestjs-mongoose) is a complete, runnable NestJS app using this adapter with an extended `user` schema and a `Post` model that `.populate()`s its author — exercised in CI, not just described in a README.
